...
Grid {
    ...
    move: Transition { NumberAnimation { properties: "x,y"; duration: 250*canvas.animated } }
    ...
    Repeater {
        ...
        Card {
            cardNo: card
            mouseEnabled: player.mouseEnabled
            y: index == cardSet.currentIndex ? 50*pickDirection : 0
            onClicked: {
                if (isValid(cardNo)) {
                    if (index != cardSet.currentIndex && canvas.pickCard) {
                        cardSet.currentIndex = index;
                    } else {
                        cardSet.currentIndex = -1;
                        player.cardClicked(index.toString(), cardNo.toString());
                    }
                } else {
